...
<html>
	...
	<body>
		...
		<h1 class='text-center text-primary'>Invoice Print</h1><hr>
		<?php
				if(isset($_POST["submit"])){
					...
					$sql="insert into invoice (INVOICE_NO,INVOICE_DATE,CNAME,CADDRESS,CCITY,GRAND_TOTAL) values ('{.rand()}','{$invoice_date}','{$cname}','{$caddress}','{$ccity}','{$grand_total}') ";
					if($con->query($sql)){
						$sid=$con->insert_id;
						...
						$sql2.=implode(",",$rows);
						if($con->query($sql2)){
							echo "<div class='alert alert-success'>Invoice Telah Ditambahkan. <a href='print.php?id={$sid}' target='_BLANK'>Tekan</a> ini untuk Print Invoice</div>";
						}else{
							echo "<div class='alert alert-danger'>Invoice Gagal Ditambahkan.</div>";
						}
					}else{
						echo "<div class='alert alert-danger'>Invoice Gagal Ditambahkan.</div>";
					}
			...
			<form method='post' action='index.php' autocomplete='off'>
				<div class='row'>
					<div class='col-md-4'>
						<h5 class='text-success'>Detail Invoice </h5>
					...
					<div class='col-md-8'>
						<h5 class='text-success'>Detail Costumer</h5>
						<div class='form-group'>
							<label>Nama</label>
